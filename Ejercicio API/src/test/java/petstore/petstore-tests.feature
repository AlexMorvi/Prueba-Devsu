...
Background:
  * url baseUrl
  * def petSchemaStrict = read('classpath:petstore/schemas/pet.schema.strict.json')
  * def petSchemaFlex = read('classpath:petstore/schemas/pet.schema.flex.json')
  * def consistencyDelayMs = testData.consistencyDelay
...
Scenario: POST /pet - Create new pet with complete data structure
    ...
    * def petPayload = buildPetData(createPetId, 'API Test Pet - ' + createPetId, 'available')
    ...
    Then status 200
    And match response == petSchemaStrict
...
Scenario: GET /pet/{petId} - Retrieve pet by unique identifier
    ...
    * def createdPet = response
    And match createdPet.id == retrievePetId
    
    # Small delay to ensure data consistency
    * sleep(consistencyDelayMs)
...
Scenario: GET /pet/findByStatus - Query pets by business status filter
    ...
    * def pets = response
    ...
    Then status 200
    And match pets == '#[] #object'
    And match each pets == petSchemaFlex
    # Validate core required fields for all pets
    And match each pets contains { "id": "#number", "status": "sold" }
    
    # Business validation: Verify our test pet is in results
    * def testPetInResults = karate.filter(pets, function(pet){ return pet.id == queryPetId })
    * assert testPetInResults.length == 1
